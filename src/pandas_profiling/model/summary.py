...
from visions.application.summaries.series import (
    ...
    url_summary,
    numerical_summary)
from visions.application.summaries.series.text_summary import length_summary, unicode_summary
...
def describe_1d(series: pd.Series) -> dict:
    ...
    def histogram_bayesian_blocks(finite_values):
        """
        Notes:
        ...
        if bayesian_blocks_bins:
            ...
            with warnings.catch_warnings():
                ...
                ret = bayesian_blocks(finite_values)
                if not np.isnan(ret).any() and ret.size > 1:
                    stats["histogram_bayesian_blocks"] = np.histogram(finite_values, ret)
        return stats
    ...
    def describe_numeric_1d(series: pd.Series, series_description: dict) -> dict:
        ...
        stats, finite_values = numerical_summary(
            series, quantiles, series_description["count"], series_description["is_unique"]
        )

        stats.update({
            "scatter_data": series,  # For complex
            "p_infinite": stats["n_infinite"] / series_description["n"],
            "p_zeros": stats["n_zeros"] / series_description["n"]
        })
        ...
        stats.update(histogram_bayesian_blocks(finite_values))
    ...
    def describe_date_1d(series: pd.Series, series_description: dict) -> dict:
        ...
        values = series[series.notna()].astype("int64").values
    ...
    def describe_categorical_1d(series: pd.Series, series_description: dict) -> dict:
        ...
        if check_length:
            ...
            stats.update(histogram_compute(stats['length'], stats["length"].nunique(), name='histogram_length'))
    ...
    def describe_url_1d(series: pd.Series, series_description: dict) -> dict:
        ...
        series = series_description['n_series']
        series = series.apply(urlsplit)

        return url_summary(series)
    ...
    def describe_file_1d(series: pd.Series, series_description: dict) -> dict:
        ...
        stats.update(describe_path_1d(series, series_description))
        stats.update(histogram_compute(stats['file_size'], stats["file_size"].nunique(), name='histogram_file_size'))
        return stats
