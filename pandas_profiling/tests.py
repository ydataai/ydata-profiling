...
class DataFrameTest(unittest.TestCase):
    ...
    def test_describe_df(self):
        ...
        expected_results = {}
        expected_results['id'] = {'25%': check_is_NaN, '5%': check_is_NaN, '50%': check_is_NaN, '75%': check_is_NaN, '95%': check_is_NaN,
                                  'count': 9, 'n_infinite': 0, 'p_infinite': 0,
                                  'cv': check_is_NaN, 'distinct_count': 9, 'freq': check_is_NaN, 'histogram': check_is_NaN,
                                  'iqr': check_is_NaN,
                                  'is_unique': True, 'kurtosis': check_is_NaN, 'mad': check_is_NaN, 'max': check_is_NaN,
                                  'mean': check_is_NaN,
        ...
        expected_results['x'] = {'25%': -0.75, '5%': -7.5499999999999989, '50%': 2.5, '75%': 23.75, '95%': 50.0,
                                 'count': 8, 'n_infinite': 0, 'p_infinite': 0, 'cv': 1.771071190261633, 'distinct_count': 7,
                                 'freq': check_is_NaN, 'iqr': 24.5,
        ...
        expected_results['cat'] = {'25%': check_is_NaN, '5%': check_is_NaN, '50%': check_is_NaN, '75%': check_is_NaN, '95%': check_is_NaN,
                                   'count': 8, 'n_infinite': 0, 'p_infinite': 0,
                                   'cv': check_is_NaN, 'distinct_count': 7, 'freq': 3, 'histogram': check_is_NaN, 'iqr': check_is_NaN,
                                   'is_unique': False, 'kurtosis': check_is_NaN, 'mad': check_is_NaN, 'max': check_is_NaN,
                                   'mean': check_is_NaN,
                                   'min': check_is_NaN, 'mini_histogram': check_is_NaN, 'mode': 'c',
                                   'n_missing': 1, 'p_missing': 0.11111111111111116, 'p_unique': 0.875,
                                   'p_zeros': check_is_NaN, 'range': check_is_NaN, 'skewness': check_is_NaN, 'std': check_is_NaN,
                                   'sum': check_is_NaN,
                                   'top': 'c', 'type': 'CAT', 'variance': check_is_NaN}
        expected_results['s1'] = {'25%': check_is_NaN, '5%': check_is_NaN, '50%': check_is_NaN, '75%': check_is_NaN, '95%': check_is_NaN,
                                  'count': 9, 'n_infinite': 0, 'p_infinite': 0,
                                  'cv': check_is_NaN, 'distinct_count': 1, 'freq': check_is_NaN, 'histogram': check_is_NaN,
                                  'iqr': check_is_NaN,
                                  'is_unique': False, 'kurtosis': check_is_NaN, 'mad': check_is_NaN, 'max': check_is_NaN,
                                  'mean': check_is_NaN,
                                  ...
                                  'range': check_is_NaN, 'skewness': check_is_NaN, 'std': check_is_NaN, 'sum': check_is_NaN,
                                  'top': check_is_NaN,
                                  'type': 'CONST', 'variance': check_is_NaN}
        expected_results['s2'] = {'25%': check_is_NaN, '5%': check_is_NaN, '50%': check_is_NaN, '75%': check_is_NaN, '95%': check_is_NaN,
                                  'count': 9, 'n_infinite': 0, 'p_infinite': 0,
                                  'cv': check_is_NaN, 'distinct_count': 1, 'freq': check_is_NaN, 'histogram': check_is_NaN,
                                  'iqr': check_is_NaN,
                                  'is_unique': False, 'kurtosis': check_is_NaN, 'mad': check_is_NaN, 'max': check_is_NaN,
        ...
        expected_results['somedate'] = {'25%': check_is_NaN, '5%': check_is_NaN, '50%': check_is_NaN, '75%': check_is_NaN,
                                        '95%': check_is_NaN,
                                        'count': 8, 'n_infinite': 0, 'p_infinite': 0, 'cv': check_is_NaN, 'distinct_count': 6,
                                        'freq': check_is_NaN,
                                        ...
                                        'skewness': check_is_NaN, 'std': check_is_NaN, 'sum': check_is_NaN, 'top': check_is_NaN,
                                        'type': 'DATE',
                                        }
            ...
            for k, v in six.iteritems(expected_results[col]):
                if v == check_is_NaN:
                    self.assertTrue(np.isnan(self.results['variables'].loc[col][k]),
                                    msg="Value {} for key {} in column {} is not NaN".format(self.results['variables'].loc[col][k], k, col))
    ...
    def test_strategies(self):
        df = self.df.copy()
        df['mixed'] = [50, 50, -10, 0, 0, 5, 15, -3, 'i am string']

        with self.assertRaises(TypeError):
            pandas_profiling.ProfileReport(df, mixed_columns_strategy='raise')

        with self.assertRaises(ValueError):
            pandas_profiling.ProfileReport(df, mixed_columns_strategy='invalid_option')

        report_after_cast = pandas_profiling.ProfileReport(df.copy(), mixed_columns_strategy='cast')
        self.assertIn('mixed', report_after_cast.description_set.get('variables').index)

        report_after_exclude = pandas_profiling.ProfileReport(df.copy(), mixed_columns_strategy='exclude')
        self.assertNotIn('mixed', report_after_exclude.description_set.get('variables').index)
        self.assertEqual(len(report_after_exclude.description_set.get('variables')), 7)
