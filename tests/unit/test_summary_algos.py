...
def test_count_summary_sorted(config):
    ...
    _, sn, r, _ = describe_counts(config, s, {})
...
def test_summary_supported_empty_df(config, empty_data):
    _, series, summary, _ = describe_counts(config, empty_data["A"], {})
    assert summary["n_missing"] == 0
    assert "p_missing" not in summary

    _, series, summary, _ = describe_generic(config, series, summary)
    assert summary["n_missing"] == 0
    assert summary["p_missing"] == 0
    assert summary["count"] == 0

    _, _, summary, _ = describe_supported(config, series, summary)
