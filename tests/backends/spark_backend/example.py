...
spark_session = (
    SparkSession.builder.appName("SparkProfiling").master("local[*]")
    .getOrCreate()
)
...
correlation_testdata = pd.DataFrame(
    {
        ...
        "test_cat_1": ["one", "one", "one", "two", "four", "four", "five", "seven", "seven", "seven", "one"],
        "test_cat_2": ["one", "one", "two", "two", "three", "four", "four", "two", "seven", None, None],
        "test_cat_3": ["one", "one", "two", "two", "three", "four", "four", "two", "one", "one", "one"],
        "test_bool": [True, False, True, False, True, False, True, False, True, False, True],
...
application = sparkmon.create_application_from_link(index=0, web_url='http://192.168.0.105:4040')

# Create and start the monitoring process
warnings.filterwarnings("ignore", category=MatplotlibDeprecationWarning)

with sparkmon.SparkMon(application, period=5, callbacks=[sparkmon.callbacks.plot_to_image]) as mon:
